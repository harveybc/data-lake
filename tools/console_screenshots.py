...
Starts the host with the disposable in-memory provider, drives a real browser over the
...
def main(argv=None) -> int:
    ...
    config.write_text(json.dumps({
        ...
        "backend": {"entry_point": "memory_store", "distribution": "data-lake-service",
                    "settings": {"store_id": "console_demo", "spool_dir": str(work / "spool"),
                                 "resources": {"prices": {
                                     "body": "time,value\n2024-01-01,1\n2024-01-02,2\n",
                                     "rows": [{"time": "2024-01-01", "value": 1},
                                              {"time": "2024-01-02", "value": 2}],
                                     "time_column": "time"}},
                                 "resource_contracts": {"prices": {
                                     "event_time_column": "time", "available_time_column": "time",
                                     "timezone": "NAIVE_WALL_CLOCK", "time_unit": None,
                                     "frequency": "1d"}}}}}))
    log = (args.out / "console.log").open("wb")
    service = subprocess.Popen([args.python, "-m", "data_lake_service.main", "--load_config", str(config)],
    ...
    try:
        ...
        with sync_playwright() as play:
            ...
            for name, path, expected in [("inventory", "/", "prices"),
                                         ("resource", "/resource?resource=prices", "event_time_column"),
                ...
                for label, viewport in VIEWPORTS.items():
                    ...
                    shot = args.out / f"lake-{name}-{label}.png"
